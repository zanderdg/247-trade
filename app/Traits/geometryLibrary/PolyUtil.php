...
namespace App\Traits\geometryLibrary;

use App\Traits\geometryLibrary\MathUtil;
use App\Traits\geometryLibrary\SphericalUtil;
...
/**
 ...
 /**
    ...
    public static function distanceToLine($p, $start, $end) {
        ...
        $s0lat = deg2rad($p[0]);
        $s0lng = deg2rad($p[1]);
        $s1lat = deg2rad($start[0]);
        $s1lng = deg2rad($start[1]);
        $s2lat = deg2rad($end[0]);
        $s2lng = deg2rad($end[1]);
        ...
        $su = [0 => $start[0] + $u * ($end[0] - $start[0]), 1 => $start[1] + $u * ($end[1] - $start[1])];
    ...
    public static function decode($encodedPath) {
        ...
        while( $index < $len) {
            ...
            array_push($path, [0 => $lat * 1e-5, 1 => $lng * 1e-5]);
        }
