                ...
                <table class="table table-bordered " id="table">
                    <thead>
                        <tr class="filters">
                            ...
                            <th width="14%">Subscription Package</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{$y}}</td>
                            <td>{!! $transactions->user_fname.' '.$transactions->user_lname !!}</td>
                            <?php
                            if($transactions->subscription_id == 1){
                                $subscription_package_name =  'DIY BAS';
                                $subscription_package_fees =  '5';
                            }elseif($transactions->subscription_id == 2){
                                $subscription_package_name =  'Express BAS';
                                $subscription_package_fees =  '29';
                            }elseif($transactions->subscription_id == 3){
                            ...
                            ?>
                            <td>{!! $subscription_package_name !!}</td>
                            ...
                            <td>@if($transactions->payment_status == 1)Paid @endif</td>
                            <td>{!! $transactions->transaction_status!!}</td>
                            <td><a href="{{ route('client/subscription-single', ['userId'=>$transactions->u_id,'nid'=>$transactions->cd_id] ) }}">View Subscription</a></td>
                        </tr>
                        <?php $y++; ?>
                        @endforeach
...
#table td, #table tr, #table th { padding: 5px; }
