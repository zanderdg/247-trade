...
<section class="content paddingleft_right15">
    ...
    <div class="row">
        <div class="the-box no-border">
            {!! Form::model($expense, array('url' => URL::to('admin/expense') . '/' .$expense->id.'/edit',
            'method' => 'post', 'class' => 'bf', 'id'=>'page_form', 'files'=> true)) !!}
            <div class="row">
                <div class="col-sm-8">
                    ...
                    <div class=" form-group">
                        ...
                        <select name="ec_id" class="form-control input-lg">
                            @foreach($expense_cats as $cats)
                                <option value="{{$cats->ec_id}}" @if($expense->ec_id == $cats->ec_id) selected @endif >{{$cats->ec_name}}
                            {{--*/ $s++; $m++; /*--}}   
                            @endforeach
                        </select>
                    </div>
...
<script src="{{ asset('assets/vendors/tags/dist/bootstrap-tagsinput.js') }}">
</script>
